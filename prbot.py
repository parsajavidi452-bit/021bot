...
class HighriseBot(BaseBot):
    ...
    async def run_ferris_wheel(self):
        ...
        try:
            ...
            users = [u for u, _ in users_res.content if not is_admin(u)]
    ...
    async def on_chat(self, user: User, message: str) -> None:
        try:
            ...
            if is_admin(user): self.admin_user_ids.add(user_id)
            ...
            if msg_low in ["!savebotloc", "!savebot", "!ثبت_بات"] and is_admin(user):
            ...
            if msg_low in ["!stopspam", "!توقف_اسپم"] and is_admin(user):
                self.stop_persistent_spam()
                await self.highrise.chat("🛑 اسپم متوقف و پاکسازی شد.")
                return

            if msg_low in ["!spamstatus", "!وضعیت_اسپم"] and is_admin(user):
            ...
            if (msg_low.startswith("!save ") or msg_low.startswith("!saveloc ")) and is_admin(user):
            ...
            if (msg_low.startswith("!delloc ") or msg_low.startswith("!removeloc ")) and is_admin(user):
                loc_name = msg.split(" ", 1)[1].strip().lower()
            ...
            if (msg_low.startswith("!tp ") or msg_low.startswith("!go ")) and is_admin(user):
            ...
            if msg_low in ["!bot", "!come", "!بیا", "!بات"] and is_admin(user):
                pos = await self.get_user_pos(user_id)
                if pos:
                    if self.bot_id:
                        await self.highrise.teleport(self.bot_id, pos)
            ...
            if msg_low.startswith("!goto ") and is_admin(user):
            ...
            if msg_low in ["!ferris", "!چرخ‌فلک", "!چرخفلک"] and is_admin(user):
            ...
            if "id=" in msg_low or "high.rs" in msg_low or msg_low.startswith("pr "):
                import re
            ...
            if msg.isdigit():
                ...
                if 1 <= num <= len(self.emotes):
                    e = self.emotes[num - 1]
                    await self.highrise.send_emote(e.id, user_id)
                return
