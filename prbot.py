...
emote_dict = {
    "Floss": ["dance-floss", 8.0, True],
    "Floss Dance": ["dance-floss", 8.0, True],
    "Rest": ["sit-idle-cute", 17.0, False],
    "Zombie": ["idle_zombie", 28.7, False],
    "Relaxed": ["sit-relaxed", 29.8, False],
    "Attentive": ["idle_layingdown", 24.5, False],
    "Sleepy": ["idle-loop-tired", 21.9, False],
    "Pouty Face": ["idle-sad", 24.3, False],
    "Posh": ["idle-posh", 21.8, False],
    "Tap Loop": ["idle-loop-tapdance", 6.2, False],
    "Sit": ["idle-loop-sitfloor", 22.3, True],
    "Shy": ["emote-shy", 4.5, True],
    "Bummed": ["idle-loop-sad", 6.0, False],
    "Chillin'": ["idle-loop-happy", 18.7, False],
    "Annoyed": ["idle-loop-annoyed", 17.0, False],
    "Aerobics": ["idle-loop-aerobics", 8.5, False],
    "Ponder": ["idle-lookup", 22.3, False],
    "Hero Pose": ["idle-hero", 21.8, False],
    "Relaxing": ["idle-floorsleeping2", 17.2, False],
    "Charging": ["emote-charging", 8.5, True],
    "Shopping Cart": ["dance-shoppingcart", 5.0, True],
    "Savage": ["emote-savage", 6.0, True],
    "Popular Vibe": ["dance-popularvibe", 8.5, True],
    "TikTok Dance 8": ["dance-tiktok8", 10.0, True],
    "TikTok Dance 2": ["dance-tiktok2", 8.5, True],
    "TikTok Dance 10": ["dance-tiktok10", 9.0, True],
    ...
    "Singing": ["idle_singing", 12.0, True],
    "Wave": ["emote-wave", 3.0, True],
    "Kiss": ["emote-kiss", 4.0, True],
    "Hug": ["emote-hug", 4.0, True],
    "Laugh": ["emote-laughing", 5.0, True],
    "Ghost": ["emote-ghost-idle", 8.0, True],
    "Super Run": ["emote-superrun", 6.0, True],
    "Super Punch": ["emote-superpunch", 5.0, True],
    "Frog": ["emote-frog", 6.0, True],
    "Cute": ["emote-cute", 4.5, True],
    "Flex": ["emote-flex", 3.5, True]
}
...
class HighriseBot(BaseBot):
    ...
    def resolve_emote_data(self, emote_key: str):
        ...
        # Handle 'floss' or 'فلوس' explicitly
        ...
        # Extract emote ID from Highrise URL link or ID format
        if "high.rs" in key_str or "item?id=" in key_str or key_str.startswith("dance-") or key_str.startswith("emote-") or key_str.startswith("idle"):
            m_id = re.search(r"id=([a-zA-Z0-9_\-]+)", key_raw)
            if m_id:
                return {"id": m_id.group(1), "name": m_id.group(1), "duration": 5.0}
            m_d = re.search(r"((?:dance|emote|idle)[a-zA-Z0-9_\-]+)", key_raw)
            if m_d:
                return {"id": m_d.group(1), "name": m_d.group(1), "duration": 5.0}

        # Exact key match in emotes_dict
        ...
        # Substring / partial match in emotes_dict
        for k, val in self.emotes_dict.items():
            if key_str in k.lower() or (isinstance(val, (list, tuple)) and key_str in str(val[0]).lower()):
                eid = val[0] if isinstance(val, (list, tuple)) else val.get("id", k)
                dur = val[1] if isinstance(val, (list, tuple)) and len(val) > 1 else 5.0
                return {"id": eid, "name": k, "duration": dur}

        # Lookup in self.emotes list
