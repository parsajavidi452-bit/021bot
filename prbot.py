...
emote_dict = {
    "Rest": ["sit-idle-cute", 17.062613, False], "Zombie": ["idle_zombie", 28.754937, False],
    "Relaxed": ["sit-relaxed", 29.889858, False], "Attentive": ["idle_layingdown", 24.585168, False],
    "Sleepy": ["idle-loop-tired", 21.959007, False], "Pouty Face": ["idle-sad", 24.377214, False],
    "Posh": ["idle-posh", 21.851256, False], "Tap Loop": ["idle-loop-tapdance", 6.261593, False],
    "Sit": ["idle-loop-sitfloor", 22.321055, True], "Shy": ["emote-shy", 4.477567, True],
    "Bummed": ["idle-loop-sad", 6.052999, False], "Chillin'": ["idle-loop-happy", 18.798322, False],
    "Annoyed": ["idle-loop-annoyed", 17.058522, False], "Aerobics": ["idle-loop-aerobics", 8.507535, False],
    "Ponder": ["idle-lookup", 22.339865, False], "Hero Pose": ["idle-hero", 21.877099, False],
    "Relaxing": ["idle-floorsleeping2", 17.253372, False], "Charging": ["emote-charging", 8.5, True],
    "Shopping Cart": ["dance-shoppingcart", 5.0, True], "Savage": ["emote-savage", 6.0, True],
    "Popular Vibe": ["dance-popularvibe", 8.5, True], "TikTok Dance 8": ["dance-tiktok8", 10.0, True],
    "Groove": ["dance-groove", 9.0, True], "Shuffle Dance": ["dance-shuffle", 8.0, True],
    "Samba": ["dance-samba", 10.0, True], "Salsa": ["dance-salsa", 9.5, True],
    "Tango": ["dance-tango", 11.0, True], "Hip Hop": ["dance-hiphop", 10.5, True],
    "Pop Dance": ["dance-pop", 8.2, True], "Rock Dance": ["dance-rock", 9.1, True],
    "K-Pop Style": ["dance-kpop", 7.8, True], "Jazz Flow": ["dance-jazz", 10.2, True],
    "Ballet Spin": ["dance-ballet", 12.0, True], "Waltz": ["dance-waltz", 11.5, True],
    "Electro Bounce": ["dance-electro", 8.8, True], "House Beat": ["dance-house", 9.3, True],
    "Techno Step": ["dance-techno", 8.7, True], "Trap Motion": ["dance-trap", 7.9, True],
    "Dubstep": ["dance-dubstep", 9.6, True], "Funk Groove": ["dance-funk", 8.4, True],
    "Soul Dance": ["dance-soul", 10.1, True], "Reggae Vibe": ["dance-reggae", 9.8, True],
    "Latino Heat": ["dance-latino", 10.3, True], "Swing Step": ["dance-swing", 8.9, True],
    "Cha Cha": ["dance-chacha", 9.2, True], "Rumba": ["dance-rumba", 11.1, True],
    "Foxtrot": ["dance-foxtrot", 10.7, True], "Quickstep": ["dance-quickstep", 8.1, True],
    "Paso Doble": ["dance-pasodoble", 10.4, True], "Jive": ["dance-jive", 8.6, True],
    "Mambo": ["dance-mambo", 9.4, True], "Bolero": ["dance-bolero", 11.8, True],
    "Country Jig": ["dance-country", 8.3, True], "Folk Dance": ["dance-folk", 9.7, True],
    "Bollywood": ["dance-bollywood", 10.6, True], "Belly Dance": ["dance-belly", 11.2, True],
    "Break Freeze": ["dance-break", 9.9, True], "Popping": ["dance-popping", 8.5, True],
    "Locking": ["dance-locking", 8.7, True], "Krump": ["dance-krump", 7.6, True],
    "Tutting": ["dance-tutting", 9.1, True], "Voguing": ["dance-voguing", 9.3, True],
    "Waacking": ["dance-waacking", 8.8, True], "Dancehall": ["dance-dancehall", 10.0, True],
    "Afrobeat": ["dance-afro", 9.5, True], "Shuffle Pro": ["dance-shuffle2", 8.2, True],
    "TikTok Dance 12": ["dance-tiktok12", 9.0, True], "TikTok Dance 13": ["dance-tiktok13", 8.6, True],
    "TikTok Dance 14": ["dance-tiktok14", 9.2, True], "TikTok Dance 15": ["dance-tiktok15", 8.8, True],
    "TikTok Dance 16": ["dance-tiktok16", 9.4, True], "TikTok Dance 17": ["dance-tiktok17", 9.1, True]
}

class Emote:
    def __init__(self, name, id, duration=5.0, is_free=True):
        self.name = name
        self.id = id
...
class HighriseBot(BaseBot):
    ...
    def resolve_emote_data(self, emote_key: str):
        if not emote_key:
            return None
        key_str = str(emote_key).strip()

        # Check if key is a digit (e.g. "5" or "12")
        if key_str.isdigit():
            num = int(key_str)
            if 1 <= num <= len(self.emotes):
                e = self.emotes[num - 1]
                return {"id": e.id, "duration": getattr(e, "duration", 5.0)}

        # Direct lookup in emotes_dict
        if key_str in self.emotes_dict:
            val = self.emotes_dict[key_str]
            if isinstance(val, (list, tuple)):
                return {"id": val[0], "duration": val[1] if len(val) > 1 else 5.0}
            elif isinstance(val, dict):
                return {"id": val.get("id", key_str), "duration": val.get("duration", 5.0)}

        # Case-insensitive lookup in emotes_dict
        for k, val in self.emotes_dict.items():
            if k.lower() == key_str.lower():
                if isinstance(val, (list, tuple)):
                    return {"id": val[0], "duration": val[1] if len(val) > 1 else 5.0}
                elif isinstance(val, dict):
                    return {"id": val.get("id", k), "duration": val.get("duration", 5.0)}

        # Lookup in self.emotes list (by id or by name)
        for e in self.emotes:
            e_id = getattr(e, "id", "")
            e_name = getattr(e, "name", "")
            if e_id == key_str or e_name.lower() == key_str.lower():
                return {"id": e_id, "duration": getattr(e, "duration", 5.0)}

        # Fallback for raw emote ID or link ID
        return {"id": key_str, "duration": 5.0}
    ...
    async def _repeat_emote(self, uid: str, emote_key: str):
        ...
        while True:
            try:
                ...
                print(f"Emote repeat error for {uid}: {ex}")
                await asyncio.sleep(2)
            ...
            if "high.rs" in msg_low or "item?id=" in msg_low or "dance-" in msg_low or "emote-" in msg_low or msg_low.startswith("pr ") or msg_low.startswith("!dance"):
                ...
                if emote_id:
                    ...
                    return
                        ...
                        # 💃 ALL ROOM DANCE (!danceall / !دنس_همه) - Owner & Admin
            if (msg_low.startswith("!danceall ") or msg_low.startswith("!دنس_همه ") or msg_low.startswith("!رقص_همه ")) and self.is_user_admin(user):
                target_d = msg.split(" ", 1)[1].strip()
                ru = await self.highrise.get_room_users()
                count = 0
                for u_item, _ in ru.content:
                    asyncio.create_task(self.start_user_dance(u_item.id, target_d))
                    count += 1
                await self.highrise.chat(f"🎉 دنس پارتی شروع شد! دنس «{target_d}» برای {count} نفر اجرا شد! 🕺💃")
                return

            if (msg_low in ["!stopdanceall", "!توقف_همه", "!استپ_همه"]) and self.is_user_admin(user):
                ru = await self.highrise.get_room_users()
                for u_item, _ in ru.content:
                    asyncio.create_task(self.stop_user_dance(u_item.id))
                await self.highrise.chat("⏹️ دنس همه کاربران متوقف شد.")
                return

            if msg_low in ["!party", "!پارتی", "!دیسکو"]:
                ru = await self.highrise.get_room_users()
                for u_item, _ in ru.content:
                    rand_num = random.randint(1, len(self.emotes))
                    e = self.emotes[rand_num - 1]
                    asyncio.create_task(self.start_user_dance(u_item.id, e.id))
                await self.highrise.chat("🔥 پارتی بزرگ شهربازی شروع شد! همه در حال رقصن! 🥳✨")
                return

            # 🤖 Direct AI Question in Chat (!ai or !سوال or !چت)
