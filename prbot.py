...
# 🛡️ ضد اسپم: هر یوزر (غیر ادمین) حداکثر هر ۱.۵ ثانیه یک‌بار می‌تونه بات رو فعال کنه.
# این جلوی هجوم پیام‌های اسپم توی روم شلوغ رو می‌گیره که باعث قطع/وصل شدن بات میشه.
_user_last_action: dict = {}
SPAM_COOLDOWN = 1.5

def _spam_allowed(user_id: str) -> bool:
    now = time.time()
    last = _user_last_action.get(user_id, 0)
    if now - last < SPAM_COOLDOWN:
        return False
    _user_last_action[user_id] = now
    if len(_user_last_action) > 500:
        cutoff = now - 60
        for k in [k for k, t in _user_last_action.items() if t < cutoff]:
            del _user_last_action[k]
    return True
...
class HighriseBot(BaseBot):
    ...
    async def on_whisper(self, user: User, message: str) -> None:
        try:
            ...
            # 🛡️ ضد اسپم توی پی‌وی هم — ادمین‌ها آزادن
            if not self.is_user_admin(user) and not _spam_allowed(user_id):
                return
    ...
    async def on_chat(self, user: User, message: str) -> None:
        try:
            ...
            is_admin_user = self.is_user_admin(user)
            if is_admin_user: self.admin_user_ids.add(user_id)
            if user_id in self.muted_users: return

            # 🛡️ ضد اسپم — ادمین‌ها محدودیت ندارن، بقیه هر ۱.۵ ثانیه یک‌بار
            if not is_admin_user and not _spam_allowed(user_id):
                return
