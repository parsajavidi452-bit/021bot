...
class HighriseBot(BaseBot):
    ...
    async def on_whisper(self, user: User, message: str) -> None:
        try:
            ...
            # 🔑 تنظیم توکن هوش مصنوعی (فقط مالک، فقط در پی‌وی — امن‌تر از چت عمومی)
            if msg_low.startswith("!setapikey ") or msg_low.startswith("!setkey ") or msg_low.startswith("!تنظیم_کلید "):
                if not self.is_owner(user):
                    await self.highrise.send_whisper(user_id, "❌ فقط مالک اصلی می‌تونه کلید هوش مصنوعی رو تنظیم کنه.")
                    return
                new_key = msg.split(" ", 1)[1].strip()
                if not new_key:
                    await self.highrise.send_whisper(user_id, "❌ راهنما: !setapikey <TOKEN>")
                    return
                try:
                    cfg_obj = {}
                    if os.path.exists(_data_path("config.json")):
                        with open(_data_path("config.json"), "r", encoding="utf-8") as cf:
                            cfg_obj = json.load(cf)
                    cfg_obj["gemini_api_key"] = new_key
                    with open(_data_path("config.json") + ".tmp", "w", encoding="utf-8") as cf:
                        json.dump(cfg_obj, cf, ensure_ascii=False, indent=2)
                    os.replace(_data_path("config.json") + ".tmp", _data_path("config.json"))
                    await self.highrise.send_whisper(user_id, "✅ کلید هوش مصنوعی ذخیره شد! الان همینجا برام «سلام» بنویس تا تست کنم.")
                except Exception as e:
                    await self.highrise.send_whisper(user_id, "❌ ذخیره کلید با خطا مواجه شد.")
                    print(f"setapikey error: {e}")
                return
